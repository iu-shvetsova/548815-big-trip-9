      ...
      <form class="event  event--edit" action="#" method="post">
        <header class="event__header">
          ...
          <div class="event__field-group  event__field-group--time">
            ...
            <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time" value="${setTime(this._startTime.getDate())}/${setTime(this._startTime.getMonth() + 1)}/${this._startTime.getFullYear().toString().substring(2)} ${setTime(this._startTime.getHours())}:${setTime(this._startTime.getMinutes())}">
            &mdash;
            <label class="visually-hidden" for="event-end-time-1">
              To
            </label>
            <input class="event__input  event__input--time" id="event-end-time-1" type="text" name="event-end-time" value="${setTime(this._endTime.getDate())}/${setTime(this._endTime.getMonth() + 1)}/${this._endTime.getFullYear().toString().substring(2)} ${setTime(this._endTime.getHours())}:${setTime(this._endTime.getMinutes())}">
          </div>
          ...
          <section class="event__section  event__section--offers">
            ...
            ${this._options.map((item) => `
              <div class="event__offer-selector">
                <input class="event__offer-checkbox  visually-hidden" id="${generateOfferName(item.name) + `-${this._number}`}" type="checkbox" name="${generateOfferName(item.name)}" ${item.isChecked ? `checked` : ``}>
                <label class="event__offer-label" for="${generateOfferName(item.name) + `-${this._number}`}">
